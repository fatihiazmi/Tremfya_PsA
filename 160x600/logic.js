...
function BuildAfterNoonTemplate(feedData,elements) {
    ...
    */
    ...
    insertCopy(elements.frame1_copy, '<span class="spanColor">Joint symptoms </span> <br>hitting hard at mid-day?','');
    insertCopy(elements.frame2_copy, 'For adults with <br>active psoriatic <br>arthritis', '');
    insertCopy(elements.frame2_copy2, `Don't let symptoms <br>define you.`, '');
    insertCopy(elements.frame3_copy, `TREMFYA<sup>®</sup> is proven to <span class="spanColor">significantly reduce</span> joint pain, stiffness, and swelling`, '');
    ...
    insertCopy(elements.frame4_copy, `Reduced PsA-related fatigue is possible`, '');
    insertCopy(elements.frame4_copy2, `Results may vary.`, '');
    insertCopy(elements.frame5_copy, `<span class="spanColor">Emerge as you</span> Ask your <br>doctor about <br>TREMFYA<sup>®</sup>`, '');
    ...
    insertImage('https://s0.2mdn.net/ads/richmedia/studio/44660889/44660889_20230927245245511_PsA_160x600_Endemic-Afternoon_Fatigue.jpg',"src", elements.heroImage);
...
/*
    //Write a Condition to Load weather template or use standard build template
    if(LoadData == ""){//do something}
        let LoadedData = loadWeatherData();
        BuildWeatherTemplate(LoadedData,feeds,element);//Pass data, feed inputs, and dom for conditionals
    }else{
        BuildTemplate(feeds,element); //Pass feed inputs, and dom for conditionals
    }
*/
    ...
    */  
        ...
        ** If you have more than 3 animation variations use linked animations with the functions included
        ...
        Set Conditionals to create different animation settings based on the shell size and animation style
            Example: if(size == "160x600" && animStyle == "Animation 1"){ //set positining/style varibales }
       ...
       //Initial Style for wrapper and main container elements should be set to display none initially.
        ...
        if (feedData[0].Person == "HM") {
            ...
            TweenMax.set(['#isi-container',],{ alpha: 0, zIndex: 0 });
            TweenMax.set(['#isi_links_hm',],{ alpha: 1 });
            TweenMax.set(['#isi_links',],{ alpha: 0, zIndex: 0 });
        }
        ...
        if (feedData[0].Person == "HM") {
        	...
        	TweenMax.to('#logo', .5, { delay: 16.8, alpha: 1, ease: Cubic.easeInOut }), // 12.8
        	TweenMax.to('#logo2', .5, { delay: 2.5, alpha: 1, ease: Cubic.easeInOut }),
        	TweenMax.to('#logo2', 0, { delay: 16.5, alpha: 0, ease: Cubic.easeInOut }), // 12.5
            //Hero  Image
            // TweenMax.to('#heroImage', .0, { alpha: 1, ease: Cubic.easeInOut }),
			TweenMax.to('#heroImage', 0.8, { delay: 4, alpha: 0, ease: Cubic.easeInOut }),
        	TweenMax.to('#heroImage', 0.8, { delay: 16.5, alpha: 1, zIndex:2, ease: Cubic.easeInOut }), // 12.5
             //Frame 1
			...
			TweenMax.to('#graphicsWrap', .5, { alpha: 1, ease: Cubic.easeInOut }),
       	    // TweenMax.to('#curveBox', 2, { alpha: 1, delay: 2, bottom:700, ease: Cubic.easeInOut }),
        
            //First Fold
			TweenMax.to('#second-fold', 1, { delay: 2, height:450, ease: Cubic.easeInOut }),
        
			TweenMax.to('#first-fold', 1, { delay: 3, height:'67%', zIndex:2, ease: Cubic.easeInOut }),
			TweenMax.to('#second-fold', 1, { delay: 5.5, alpha: 0, height:450, ease: Cubic.easeInOut }),
    
            //System  graphics
            TweenMax.to('#graphicImg1', 1, { delay: 3, bottom:135, alpha: 1,  ease: Cubic.easeInOut }),
            TweenMax.to('#graphicImg1', 12.5, { delay: 2.5, rotate:-65, ease: Cubic.easeInOut }),
        	// TweenMax.to('#graphicImg1', 1.5, { delay: 8, rotate:0, ease: Cubic.easeInOut }),
           
            TweenMax.to('#graphicImg1', .5, { delay: 17, alpha: 0, ease: Cubic.easeInOut }), // 13
            TweenMax.to('#graphicImg1', 1, { delay: 16.3, alpha: 1, bottom:'150%', scale:2.8, ease: Cubic.easeInOut }), // 12.3
            TweenMax.to('#graphicImg2', 1, { delay: 16.8, alpha: 1, scale:1.04, bottom:230, ease: Cubic.easeInOut }), // 12.8
           
            // //Frame 3
            TweenMax.to('#frame3', 1, { alpha: 1, delay: 5.2, ease: Cubic.easeInOut }),
            TweenMax.to('#frame3', 1, { alpha: 0, delay: 10.5, ease: Cubic.easeInOut }), // 8.5
    
            // //Frame 4
            TweenMax.to('#frame4', 1, { alpha: 1, delay: 10.5, ease: Cubic.easeInOut }), // 8.5
            TweenMax.to('#frame4', 1, { alpha: 0, delay: 16, ease: Cubic.easeInOut }), // 12
    
            // //Frame 5
			TweenMax.to('#frame5', 1, { delay: 1, y: "+=20", ease: Cubic.easeInOut }),
            TweenMax.to('#frame5', 1, { alpha: 1, delay: 17.2, y: "-=20", ease: Cubic.easeInOut }), // 13.2
    
            //  //Cta
			TweenMax.to('#cta', 1, { delay: 1, y: "+=20", ease: Cubic.easeInOut }),
            TweenMax.to('#cta', 1, { alpha: 1, delay: 17.2, y: "-=76", ease: Cubic.easeInOut }), // 13.2 
                
            ]);
            //console.log('Animation Complete');
        } else {

        TweenMax.set(['#isi-container-hm',],{ alpha: 0, zIndex: 0 });
        TweenMax.set(['#isi-container',],{ alpha: 1 });
        TweenMax.set(['#isi_links_hm',],{ alpha: 0, zIndex: 0 });
        TweenMax.set(['#isi_links',],{ alpha: 1 });
        ...
        timeline.add([
            ...
            //System  graphics
            TweenMax.to('#graphicImg1', 1, { delay: 3, bottom:99, alpha: 1,  ease: Cubic.easeInOut }),
            TweenMax.to('#graphicImg1', 12.5, { delay: 2.5, rotate:60, ease: Cubic.easeInOut }),
        	// TweenMax.to('#graphicImg1', 1.5, { delay: 8, rotate:0, ease: Cubic.easeInOut }),
           
            TweenMax.to('#graphicImg1', .5, { delay: 17, alpha: 0, ease: Cubic.easeInOut }), // 13
            TweenMax.to('#graphicImg1', 1, { delay: 16.3, alpha: 1, bottom:'150%', scale:2.8, ease: Cubic.easeInOut }), // 12.3
            TweenMax.to('#graphicImg2', 1, { delay: 16.8, alpha: 1, scale:1, bottom:0, ease: Cubic.easeInOut }), // 12.8
            ...
            TweenMax.to('#frame5', 1, { alpha: 1, delay: 17.2, y: "-=20", ease: Cubic.easeInOut }), // 13.2
             ...
             //Cta
			TweenMax.to('#cta', 1, { delay: 1, y: "+=20", ease: Cubic.easeInOut }),
            TweenMax.to('#cta', 1, { alpha: 1, delay: 17.2, y: "-=20", ease: Cubic.easeInOut }), // 13.2
